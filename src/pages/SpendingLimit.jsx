...
function SpendingLimit() {
  ...
  useEffect(() => {
    axios
      .get('http://localhost:3001/limits')
      .then((res) => {
              ...
              <div className="spending-detail-list">
                <div className="spending-category">
                  <div className="spending-icon">
                    ...
                    <img src={"http://localhost:3001/icons/"+spending.Category.Icon.content} alt="icon"/>
                  </div>
                  <div className="spending-info">
                    <div className="spending-info-category">{spending.Category.name}</div>
                  </div>
                </div>
                <div>
                  <div className="price">{spending.limit_money} đ</div>
                  <div style={{marginTop: '5px', color: 'gray', fontWeight: '600'}}>{spending.date}</div>
                </div>
              </div>
            ))}
      </div>
